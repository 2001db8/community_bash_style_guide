...
pgrep ${processname}
```
In the rare case that you really need the whole output of `ps` for a process versus only the PID when using `pgrep` you can put the first letter of the process name you are seraching for in square brackets:
```bash
$ ps au | grep bash
